...
function cleanText(text) {
    let cleanText = text.slice(0, text.indexOf("<"));
   let cleanerText = cleanText.slice(0,cleanText.indexOf("―"));
   cleanerText = cleanerText.split('').filter(item => {
        return ((item !== '“') && (item !== '”'));
    });
   return cleanerText.join('');
}
 
async function getText(topic) {
   return new Promise((resolve, reject) => {
       let contents = [];
       osmosis.get(`https://www.goodreads.com/quotes/tag/${topic}`) // Crawl this webpage
           .find("div.quoteText") // Get all elements matching this CSS selector
...
// async function haikuPoem(topic) {
//    // 1. Get the text
//    const paragraphs = await getText(topic);
//    let fragments = [];
...
// async function rhymingPoem(topic) {
//    const paragraphs = await getText(topic);
...
//             const fragment = lexes.filter(item => {
//                 return item !== '.';
//              });
//             if(fragment.length>0) storeFragment(fragment);
...
//    // This function gets a rhyming couplet or triplet
//    function getRhymingLines(goodKeys, rhymeGroups, numberOfLines) {
//        scramble(goodKeys);
//        const rhymingFragments = rhymeGroups[goodKeys[0]];
//        scramble(rhymingFragments);
//        const fragment1 = rhymingFragments[0];
//        let fragment2, fragment3;
//        let fragmentsWithADifferentLastWord;
//        let moreFragmentsWithADifferentLastWord;
//        try {
//            if(numberOfLines<2 || numberOfLines>3) throw "Rhyme group must be two or three lines"
//        }
//        catch(err){
//            return err;
//        }
     
//                fragmentsWithADifferentLastWord = rhymingFragments.filter(fragment => {
//                    return fragment.slice(-1)[0].toLowerCase() !== fragment1.slice(-1)[0].toLowerCase();
//                });
//                scramble(fragmentsWithADifferentLastWord);
//                fragment2 = fragmentsWithADifferentLastWord[0];
//                return [fragment1.join(" "), fragment2.join(" ")];
  
//        if(numberOfLines === 3) {
//                 moreFragmentsWithADifferentLastWord = rhymingFragments.filter(fragment => {
//                    return (fragment.slice(-1)[0].toLowerCase() !== fragment1.slice(-1)[0].toLowerCase()) && (fragment2.slice(-1)[0].toLowerCase() !== fragment1.slice(-1)[0].toLowerCase() );
//                });
//                scramble(fragmentsWithADifferentLastWord);
//                fragment3 = fragmentsWithADifferentLastWord[0];
//                return [fragment1.join(" "), fragment2.join(" "), fragment3.join[" "]];
//            }       
...
//        const tripletA = getRhymingLines(goodKeys, rhymeGroups, 3);
//        const coupletB = getRhymingLines(goodKeys, rhymeGroups, 2);
      
//        lines.push(tripletA[0]);
//        lines.push(tripletA[1]);
//        lines.push(coupletB[0]);
//        lines.push(coupletB[1]);
//        lines.push(tripletA[2]);
...
async function getLine(topic) {
    ...
    fragments.filter(fragment => syllable(fragment) <= 12);
    // 4b. Scramble the fragment arrays
    scramble(fragments);
    return [fragments[0]];
 }
 
async function makePoem(topic) {
   return await getLine(topic);
}
 
// if (require.main === module) {
//    // getText();
//    makePoem('life').then(res => console.log(res));
// }
